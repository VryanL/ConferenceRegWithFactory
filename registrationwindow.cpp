    ...
    : QMainWindow(parent)
{
    ...
    saveB = new QPushButton("&SaveXML");
    QPushButton *readB = new QPushButton("&LoadXML");
    bottomBut->addSpacerItem(spacerBot);
    bottomBut->addWidget(readB,0,Qt::AlignRight);
    bottomBut->addWidget(saveB,0,Qt::AlignRight);
    bottomBut->addWidget(quitB,0,Qt::AlignRight);
    ...
    connect(saveB, SIGNAL(clicked(bool)), this, SLOT(writeToXML()));
    connect(readB, SIGNAL(clicked(bool)), this, SLOT(readXML()));
...
void RegistrationWindow::updateTable() const
{
    ...
    if (rList.getAttendeeList().isEmpty())
        saveB->setEnabled(false);
    else
...
void RegistrationWindow::addRegistration()
{
    ...
    if (getUserinfo.exec() == QDialog::Accepted)
    {
        ...
        {
            if (rList.addRegistration(regFactory->createRegistration("Registration", person))){
        ...
        {
            if (rList.addRegistration(regFactory->createRegistration("StudentRegistration", person,getUserinfo.getExtraInfo()))){
                QMessageBox::information(this, "Registered", QString("\'%1\' was added").arg(getUserinfo.getName()));
            }
        }
        else if (getUserinfo.getType() == "GuestRegistration")
        {
            if (rList.addRegistration(regFactory->createRegistration("GuestRegistration", person,getUserinfo.getExtraInfo()))){
                QMessageBox::information(this, "Registered", QString("\'%1\' was added").arg(getUserinfo.getName()));
            }
        }
        updateTable();
...
void RegistrationWindow::writeToXML()
{
    QString filePath = QFileDialog::getSaveFileName(this, "Save XML file", "", "XML files(*.xml);;All files(*.*");

    if(!filePath.isEmpty())
    {
        RegistrationListWriter writeXML(rList);
        if (writeXML.writeToXml(filePath))
            QMessageBox::information(this, "XML Save", "List was saved successfully!");
        else
            QMessageBox::warning(this, "XML Save", "List save was unsuccessful!");
    }
    registerB->setFocus();
}

void RegistrationWindow::readXML()
{
    QString fileName = QFileDialog::getOpenFileName(this, "Open XML File", "", "XML Files (*.xml)");

    if (!fileName.isEmpty())
    {
        QList<Registration*> newRegistrations = xmlReader->parseXML(fileName);
        for (Registration *reg : newRegistrations)
        {
            rList.addRegistration(reg);
        }
        updateTable();
    }
    registerB->setFocus();
